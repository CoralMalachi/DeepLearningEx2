...
#import torch for using tensors and more functions
import torch
...
class Part1Model(object):
    ...
    def train_neural_network(self, epoch, avg_train_loss_per_epoch_dict):
        ...
        length = BATCH_NORMALIZATION_SIZE * len(self.training_set)
        train_loss /= (len(self.training_set))
        #add train loss to our list
        avg_train_loss_per_epoch_dict[epoch] = train_loss
        # compute accuracy
        accuracy = 100. * m_success / (length)
        #print message
    ...
    def feedforward_the_dev_set(self, iter_number, dev_loss_dictionary,
        ...
        #add the loss into the dev list of losses
...
# Function Action: create the paths for files
################################################################
